# This is a Inventory Project made in java
 **it's 2 separate Inventory for Goods and Medicine**
 - has mac OS-Installer
 - has Windows OS-Installer
 - has Linux Installer

**never tried Installing in other OS tho since i only use Linux**

# You can Donate Through Gcash :)
- Number: **09763707528**
- Name: Francis Kenjie Maraasin
- Thank you very much

# Step by Step Install

**Step 1** go to this link - https://github.com/CodeKenjie/InventoryApp/releases/tag/v1.0

**Step 2** install the (.zip or tar.zip) file it will install a zip file

**Step 3** unzip the file or just pick the intaller you need for your OS 
- for Windows(.exe) 
- for MacOS(.dmg)
- for Linux (.deb)

this will install the file and i believe it should make a shortcut in your Start Menu (Sorry this is my first time releasing an app, so im not sure about most of the things)

**Step 4** Use the Your inventory system :)

thats all thank you very much

cttm: CodeKenjie
