...
public class Medicine extends JFrame implements ActionListener{
    ...
    String[] cols = {"Id", "Date", "Medicine Name", "Category", "Quantity", "Notes", "Expiration Date"};
    ...
    Medicine(){
        ...
        JLabel item_l = new JLabel("Medicine:");
        item_l.setBounds(22, 13, 100, 24);
        ...
        this.setTitle("Medicine InventorySystem");
    ...
    public void loadDB(DefaultTableModel model){
        try{
            ...
            ResultSet rs = stat.executeQuery("SELECT * FROM medicines");

            while (rs.next()) {
                model.addRow(new Object[]{rs.getInt("id"), rs.getString("Date"), rs.getString("Medicine"), rs.getString("Category"), rs.getInt("Quantity"), rs.getString("Note"), rs.getString("Expiration")});
            }
        ...
        if (e.getSource() == add) {
            ...
            if (item_tf.getText().equals("")){
                ...
                try {
                    Class.forName(jdbc);
                    Connection conn = DriverManager.getConnection(url);
                    PreparedStatement prep = conn.prepareStatement("INSERT INTO medicines (Date, Medicine, Category, Quantity, Note, Expiration) VALUES(?, ?, ?, ?, ?, ?)"); 
                ...
                try {
                    Class.forName(jdbc);
                    Connection conn = DriverManager.getConnection(url);
                    PreparedStatement prep = conn.prepareStatement("INSERT INTO medicinelogs(Date, Name, Medicine, Action, Quantity) VALUES(?, ?, ?, ?, ?)");
                    prep.setString(1, cdate);
            ...
            if (selrow > -1) {
                ...
                if (confirm == 0) {
                    try {
                        Class.forName(jdbc);
                        Connection conn = DriverManager.getConnection(url);
                        PreparedStatement prep = conn.prepareStatement("DELETE FROM medicines WHERE id = ?");
        ...
        if (e.getSource() == update) {
            ...
            if (selrow > -1) {
                try {
                    Class.forName(jdbc);
                    Connection conn = DriverManager.getConnection(url);
                    PreparedStatement prep = conn.prepareStatement("UPDATE medicines SET Date = ?, Medicine = ?, Category = ?, Note = ?, Expiration = ? WHERE id = ?");
        ...
        if (e.getSource() == imprt) {
            ...
            if (selrow > -1) {
                try {
                    Class.forName(jdbc);
                    Connection conn = DriverManager.getConnection(url);
                    PreparedStatement prep = conn.prepareStatement("UPDATE medicines SET Quantity = ? WHERE id = ?");
                    ...
                    quantity_sp.setValue(0);
                    JOptionPane.showMessageDialog(null, String.valueOf(currentQuant) + " item/s are Imported to " + String.valueOf(ItemTable.getValueAt(selrow, 2)));
                } catch(ClassNotFoundException err){
                    err.printStackTrace();
                } catch(SQLException err){
                    err.printStackTrace();
        ...
        if (e.getSource() == exprt) {
            ...
            if (selrow > -1 && currentQuant <= selQuantity) {  
                ...
                try {
                    ...
                    quantity_sp.setValue(0);
                    JOptionPane.showMessageDialog(null, String.valueOf(currentQuant) + " item/s are Exported from " + String.valueOf(ItemTable.getValueAt(selrow, 2)));
                } catch(ClassNotFoundException err){
                    err.printStackTrace();
                } catch(SQLException err){
                    err.printStackTrace();
        ...
        if (e.getSource() == search) {
            ...
            if (!key.equals("")) {
                try {
                    ...
                    Connection conn = DriverManager.getConnection(url);
                    Statement stat = conn.createStatement();
                    PreparedStatement prep = conn.prepareStatement("SELECT * FROM medicines WHERE id = ? OR Date LIKE ? OR Medicine LIKE ? OR Category LIKE ? OR Quantity = ? OR Note LIKE ? OR Expiration LIKE ?;");
